...
const operadores = (oper) => {
    if(display.textContent == ''){
        display.textContent = ''
    } else if(numeroAnterior == undefined){
        numeroAnterior = display.textContent
        operador = oper
        operando2.textContent = oper
        gatilho = false

    }else{
        display.textContent = parseFloat(eval(`${numeroAnterior}${operador}${display.textContent}`))
        numeroAnterior = display.textContent
        operador = oper
        operando2.textContent = oper
        gatilho = false
